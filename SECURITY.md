...
This is the list of versions of `HMS` which are
...
To report a vulnerability please send an email with the details to mwakidenice@gmail.com. The vulnerability report must include a proof-of-concept of the exploit, or at least a few pointers that can help us assess the risk level.
...
2. the vulnerability affects Healthcare Management System usage of that dependency.

Note that if (1) is true but (2) is false, we will consider it as a low-level vulnerability and might decide not to fix it.

---

Thanks for helping to keep Heathcare App secure.
